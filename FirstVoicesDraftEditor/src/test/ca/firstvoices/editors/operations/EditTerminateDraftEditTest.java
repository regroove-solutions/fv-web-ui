...
public class EditTerminateDraftEditTest
{
    ...
    public void editTerminateDraftEditTest()
    {
        ...
        testUtil.terminateDraftEditViaAutomation( automationService, draftEditorServiceInstance, docArray );

        session.save();

        docArray = testUtil.getTestWordsArray( session );

        assertTrue("Should have only live documents after draft session was terminated", docArray.length == initialWordCount);
