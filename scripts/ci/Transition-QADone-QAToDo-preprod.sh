...
# This script will perform the transitions when a RC release is performed.
...
do
    ...
    if [[ $? -eq 0 ]]; then
        ...
        if [[ $(echo $INFO | grep -o 'labels:[[:space:]].*[[:space:]]description:') =~ DEPLOYED-DEV && ! $(echo $INFO | grep -o 'labels:[[:space:]].*[[:space:]]description:') =~ DEPLOYED-PREPROD ]]; then
            LABEL=true
            echo $i ': DEPLOYED-DEV label found successfully and no DEPLOYED-PREPROD label found.'
        fi
    ...
    # If The issue has been QA'd on dev then transition to QA To DO and add a label or else
    # give some output saying why it was not transitioned.
    if [[ $LABEL = true && $STATUS = true ]]; then
        echo $i ": Transitioning - QA DONE -> QA TO DO."
        jira transition --noedit "QA TO DO" $i
        echo $i ": Adding DEPLOYED-PREPROD label to issue."
        jira labels add $i DEPLOYED-PREPROD
        echo $i ": Adding comment to issue."
        jira comment --noedit --comment="This issue has been QA'd on dev and will be deployed to preprod.firstvoices.com." $i
    fi
    if [[ $FOUND = false ]]; then
        echo $i ': No transition performed. Issue key could not be found on Jira.'
    else
        if [[ $LABEL = false ]]; then
            echo $i ': No transition performed. The DEPLOYED-DEV label does not exist or the DEPLOYED-PREPROD label was found. '
        fi
