...
export default class PageHome extends Component {
  static propTypes = {
    ...
    fetchUserDialect: PropTypes.func.isRequired,
    computeUserDialect: PropTypes.object.isRequired,
  ...
  componentWillReceiveProps(nextProps) {
    const USER_LOG_IN_STATUS_CHANGED =
      nextProps.computeLogin.isConnected &&
      !this.props.computeLogin.isConnected &&
      nextProps.computeLogin.isConnected != undefined
    let computeUserDialectPath

    // Redirect user to their own dialect
    if (USER_LOG_IN_STATUS_CHANGED && this.props.windowPath == "/") {
      this.props.fetchUserDialect()
    }

    if (nextProps.computeUserDialect.size > 0) {
      let computeUserDialect = nextProps.computeUserDialect.get(0)
      computeUserDialectPath = selectn("entries[0].path", computeUserDialect.get("response"))
    }

    if (computeUserDialectPath) {
      window.location.replace(this.props.windowPath + "explore" + computeUserDialectPath)
    }
  }
