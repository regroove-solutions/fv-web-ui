...
var handler = ''
var submitHandler = ''
export function setHandler(handleChange, handleSubmit){
...
var languageGuess = {
  ...
  enabled: true,
  main: <GuessPg name='languageGuessAnswer' />,
}
var languagePick = {
  title: 'Langauge',
...
var familyGuess = {
  ...
  enabled: true,
  main: <GuessPg name='familyGuessAnswer' />,
}
var familyPick = {
  title: 'Language Family',
...
var alphabetGuess = {
  ...
  enabled: true,
  main: <GuessPg name='alphabetGuessAnswer'/>,
}
var alphabetPick = {
  title: 'Alphabet',
...
function StepsPg(props) {
  return <Steps steps={["start", "enter info", "longer sentence that will hopefullly need to wrap around and potentially cause problems add stuff", "create"]} />;
}
...
function GuessPg(props){
  return <DropDown name={props.name} title='Is guess correct?' options={['correct', 'incorrect']} handleChange={handler} />;
}

function PickPg(props){
  return <DropDown name={props.name} title='Please choose own' options={['generated', 'options', 'from', 'db', 'other']} handleChange={handler} />;
}
...
function DialectInfoPg(props){
  return (
    <div>
      <DropDown name='country' title='country' options={['generated', 'options', 'from', 'db', 'other']} handleChange={handler}/>
      <div style={{marginTop:'20px'}}/>
      <DropDown name='region' title='region' options={['generated', 'options', 'from', 'db', 'other']} handleChange={handler}/>
    </div>
  );
}
//EnterText other popup
...
function KeyboardsPg(props) { 
  return(
    <div>
      <DropDown name='keyboard' title='keyboards' options={['generated', 'options', 'from', 'db']} handleChange={handler}/>
      <div style={{marginTop:'20px'}}/>
      <DropDown name='keyboardGuide' title='keyboard guides' options={['generated', 'options', 'from', 'db']} handleChange={handler}/>
    </div>
  );
}
//multi select
...
function LangAdminAskPg(props) {
  return <DropDown name='langAdminAskAnswer' title='Already have account?' options={['yes', 'no']} handleChange={handler} />;
}
...
function CreationPg(props){
  return <SubmitButton onSubmit={submitHandler}/>;
}
