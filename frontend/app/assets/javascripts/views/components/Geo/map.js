...
export default class Index extends Component {
  constructor(props, context) {
    ...
    this.state = {
      ...
      scale: 1,
      zoom: 1,
      currentBounds: [[83.100354, -144.257421], [40.677028, -47.82826]],
    }
  ...
  render() {
    ...
    // minZoom={2.2}
    return (
      <Map center={position} zoom={this.state.zoom} bounds={this.state.currentBounds}>
        <GeoJSON
        ...
        {(this.props.dialects || []).map(function(dialect, i) {
          let myIcon = L.icon({
            iconUrl:
              UIHelpers.getThumbnail(selectn('contextParameters.portal.fv-portal:logo', dialect), 'Thumbnail') ||
          ...
          let coords = selectn('properties.fv-portal:map_marker_coords', dialect)
            .split(',')
            .map((v) => parseInt(v))
          ...
          if (coords.length > 1) {
            return (
              <Marker key={i} icon={myIcon} position={coords}>
                <Popup>
                  <span>
                    {selectn('contextParameters.ancestry.dialect.dc:title', dialect)}
                    <br />
                    {location}
                    <br />
                    <a href={'/explore' + selectn('contextParameters.ancestry.dialect.path', dialect)}>
                      {intl.trans('views.components.go_to_dialect', 'Go to dialect', 'first')}
                    </a>
                  </span>
                </Popup>
              </Marker>
