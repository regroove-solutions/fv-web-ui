...
export default class AudioOptimal extends Component {
  ...
  _getDescription(metadata) {
      return (metadata.description ? <span className="AudioOptimalMetadataRow"><span className="AudioOptimalMetadataLabel">DESCRIPTION</span> {metadata.description}</span> : "");
  }
  ...
  _getSpeakers(metadata) {
    ...
    (metadata.speakers || []).map(function (speaker) {
        if (speaker) {
            speakersCont.push(<span className="AudioOptimalMetadataRowValue" key={selectn("uid", speaker)}>{selectn("dc:title", speaker)}</span>);
        }
    });

    return speakersCont.length > 0 ? <span className="AudioOptimalMetadataRow"><span className="AudioOptimalMetadataLabel">SPEAKER(S)</span>{speakersCont}</span> : null
  }
  ...
  _getRecorders(metadata) {
    ...
    (metadata.recorders || []).map(function (recorder) {
        if (recorder) {
            recordersCont.push(<span className="AudioOptimalMetadataRowValue" key={selectn("uid", recorder)}>{selectn("dc:title", recorder)}</span>);
        }
    });

    return recordersCont.length > 0 ? <span className="AudioOptimalMetadataRow"><span className="AudioOptimalMetadataLabel">RECORDED BY</span>{recordersCont}</span> : null
  }
  ...
  render() {
    ...
    return (
        <div className="AudioOptimal">{this.props.audioTag}<IconButton
