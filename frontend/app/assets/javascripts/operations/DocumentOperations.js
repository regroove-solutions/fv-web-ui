...
export default class DocumentOperations extends BaseOperations {
    ...
    return new Promise((resolve, reject) => {
      const defaultParams = {
        query: ` SELECT * FROM ${documentList.model.prototype.entityTypeName}  WHERE (fva:dialect = '${dialect.get(
          "id"
        )}' AND ecm:currentLifeCycleState <> 'deleted') ${query ? `AND ${query}` : ""} ORDER BY dc:title`,
      }
  ...
  static executeOperation(input, operationName, operationParams, headers = {}, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // If you explicitly want NO properties, force no properties to be returned
      if (selectn("headers.X-NXProperties", headers) === "") {
        operation.header("X-NXproperties", "")
      }
  ...
  static getCharactersByDialect(path, headers = {}, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      const defaultParams = {
        query: `SELECT * FROM FVCharacter WHERE (ecm:path STARTSWITH '${cleanedDialectPath}' AND ecm:currentLifeCycleState <> 'deleted') ORDER BY fvcharacter:alphabet_order ASC`,
      }
  ...
  static queryDocumentsByDialect(path, queryAppend, headers = {}, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      const defaultParams = {
        query: `SELECT * FROM Document WHERE (ecm:path STARTSWITH '${cleanedDialectPath}' AND ecm:currentLifeCycleState <> 'deleted')${queryAppend} ORDER BY dc:title ASC`,
      }
  ...
  static searchDocuments(queryParam, queryPath, docTypes, headers = {}, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      const defaultParams = {
        query: `SELECT * FROM Document WHERE (ecm:path STARTSWITH '${queryPath}' AND ecm:currentLifeCycleState <> 'deleted') AND ecm:primaryType IN (${docTypes}) AND ecm:fulltext = '*${queryParam}*' ORDER BY dc:title ASC`,
      }
