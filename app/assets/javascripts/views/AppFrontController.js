...
const PAGE_NOT_FOUND_TITLE = '404 - ' + intl.translate({
  key: 'errors.page_not_found',
  default: 'Page Not Found',
  case: 'first',
})

const PAGE_NOT_FOUND_BODY = <div>
  <p>{intl.translate({
...
export default class AppFrontController extends Component {
    ...
    _updateTitle() {
      ...
      if (title.search(' | ') >= 0) {
        const newTitle = []; const parts = title.split('|'); let i
        for (i in parts) {
    ...
    _renderBreadcrumb(matchedPage, routeParams) {
      ...
      const breadcrumb = splitPath.map(function(path, index) {
        ...
        <div>
          <div className="row">
            <div className="clearfix" style={{backgroundColor: themePalette.accent4Color}}>
              ...
              <ol className={classNames('breadcrumb', 'pull-left', 'AppFrontControllerBreadcrumb')}>
    ...
    _matchPath(pathMatchArray, urlPath) {
      ...
      const matched = pathMatchArray.every(function(value, key) {
    ...
    render() {
      ...
      let page; let navigation = <Navigation frontpage={isFrontPage} routeParams={matchedRouteParams} />
      const theme = (matchedRouteParams.hasOwnProperty('theme')) ? matchedRouteParams.theme : 'default'
      const print = (matchedPage) ? matchedPage.get('page').get('props').get('print') === true : false
      ...
      return (
        <div>
          ...
          <div className="row AppFrontControllerHeader">{navigation}</div>
          <div id="pageContainer">{page}</div>
          <div className="row AppFrontControllerFooter">{footer}</div>
        </div>
      )
    }
}
