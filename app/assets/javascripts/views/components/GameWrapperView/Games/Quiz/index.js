...
class Quiz extends React.Component {
  ...
  constructor(props) {
    ...
    WordOperations.getWordsByLangauge(props.client, props.language, "(dc:subjects LIKE '%biology')").then((function(words){
    ...
    PubSub.subscribe( 'QUIZ:DATALOADED', this.displayAnswers );
  }
  ...
  render() {
    ...
    var LinearProgressStyle = {height: '15px', marginBottom: '10px'};

    return <div className="quiz-container">
      <div className="row">
        <div className="col-xs-12">
          <LinearProgress style={LinearProgressStyle} mode="determinate" value={((this.state.currentAnswerIndex + 1) / this.totalQuestion) * 100} />
        </div>
      </div>      
      <div className="row">
        <div className="col-xs-12">
      ...
      <div className={classNames('row', 'row-navigation')}>
        <div className={classNames('col-xs-2', 'text-left')}>
          <IconButton onTouchTap={this.handleTouchTap.bind(this, 'previous')} iconClassName={classNames('glyphicon', 'glyphicon-chevron-left')} tooltip="Previous Question"/>
        </div>
        <div className={classNames('col-xs-8', 'text-center')}>
          ...
          <Snackbar ref="feedback" message={(this.state.checkedAnswers[this.state.currentAnswerIndex] === true) ? 'Great job!' : (this.state.currentAnswerIndex in this.state.checkedAnswers) ? 'Try Again...' : ''} action="close" autoHideDuration={1500} />
        </div>
        <div className={classNames('col-xs-2', 'text-right')}>
          <IconButton onTouchTap={this.handleTouchTap.bind(this, 'next')} iconClassName={classNames('glyphicon', 'glyphicon-chevron-right')} tooltip="Next Question"/>
        </div>
      </div>
    </div>;
  }
