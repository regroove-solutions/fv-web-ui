...
export default class Navigation extends Component {
  ...
  render() {
    ...
    const portalTitle = selectn('response.contextParameters.ancestry.dialect.dc:title', computePortal) || selectn('response.properties.dc:title', computeDialect)
    ...
    return <div>
      <AppBar
        ...
        <ToolbarGroup style={{position: 'relative', color: '#fff'}}>
          ...
          <div style={{display: 'inline-block', paddingRight: '10px', paddingTop: '15px', textTransform: 'uppercase'}}>
            <Link className="nav_link" href={'/explore' + this.state.pathOrId + '/Data'}>{this.intl.trans('choose_lang', 'Choose a Language', 'first')}</Link>
          </div>
      ...
      <Toolbar style={{display: (this.state.localePopoverOpen) ? 'block' : 'none' }}>
        <ToolbarGroup firstChild float="right">
          <ToolbarTitle style={{'color': '#fff', 'padding': '0 0 0 15px', 'fontSize': '15px'}} text={this.intl.trans('choose_lang', 'Choose a Language', 'first')} />
      ...
      { isDialect && (
        <div className="row" style={{backgroundColor: themePalette.primary2Color, minHeight: '64px', margin: '0'}}>
          <h2 className="NavigationDialectHeader">
            <a className="NavigationDialectLink" onTouchTap={this._onNavigateRequest.bind(this, '/explore' + this.props.routeParams.dialect_path)}>
