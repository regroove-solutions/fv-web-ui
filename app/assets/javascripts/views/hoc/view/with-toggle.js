    ...
    render() {
        ...
        return <div className={classNames('panel', 'panel-default')}>
                    <div className="panel-heading">
                        {label} <FlatButton className={classNames({'visible-xs': mobileOnly})} label={(this.state.open) ? 'Hide' : 'Show'} labelPosition="before" onTouchTap={(e) => {this.setState({open: !this.state.open}); e.preventDefault(); }} icon={<FontIcon className="material-icons">{(this.state.open) ? 'expand_less' : 'expand_more'}</FontIcon>} style={{float: 'right', lineHeight: 1}} />
                    </div>
