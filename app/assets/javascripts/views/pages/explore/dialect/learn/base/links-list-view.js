...
export default class ListView extends DataListView {
  ...
  constructor(props, context) {
    ...
    this.state = {
      columns : [
        { name: 'title', title: 'Link', render: function(v, data, cellProps){ return v; }},
        { name: 'dc:description', title: 'Description', render: function(v, data, cellProps) {
          return selectn('properties.dc:description', data);
        }},
        { name: 'fvlink:url', title: 'URL', render: function(v, data, cellProps) {
          return selectn('properties.fvlink:url', data);
        }},
        { name: 'thumb:thumbnail', width: 72, textAlign: 'center', title: 'File', render: function(v, data, cellProps) {
            let filePreview = selectn('properties.thumb:thumbnail.data', data);
            if (filePreview)
              return <img style={{maxWidth: '62px', maxHeight: '45px'}} key={selectn('uid', filePreview)} src={UIHelpers.getThumbnail(filePreview, 'Thumbnail')} />;
          }.bind(this)
        }
      ],
      sortInfo: {
