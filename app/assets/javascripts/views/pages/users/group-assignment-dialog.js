...
export default class GroupAssignmentDialog extends Component {
  ...
  render() {
    ...
    let currentlyAssignedGroupsLabels = (currentlyAssignedGroups).map((group) => { return <span className={classNames('label', 'label-default')} style={{marginRight: '5px'}}>{StringHelpers.toTitleCase(group.replace(/_/g, ' '))}</span> })
    ...
    let formOptions = {
        fields: {
            ...
            'group': {
                label: (this.isUserRegistration) ? 'Group to Add User to:' : 'Groups:',
                help: (this.isUserRegistration) ? 'Note: Only one group per user (for each dialect) is required due to permission inheritance.' : 'Note: Groups with more permissions will inherit permissions from groups with less permissions. Example: \'Recorders with Approval\' get permissions that both \'Recorders\' and a \'Members\' have.)',
                disableOrder: true,
                item: {
    ...
    return <Dialog
				...
				<form onSubmit={this._onRequestSaveForm}>
                    <t.form.Form
                        ...
                        value={{'id': selectn(this.props.fieldMapping.id, this.props.selectedItem), 'group': currentlyAssignedGroups}}
                        type={formSchema}
                        options={formOptions} />
                </form>
