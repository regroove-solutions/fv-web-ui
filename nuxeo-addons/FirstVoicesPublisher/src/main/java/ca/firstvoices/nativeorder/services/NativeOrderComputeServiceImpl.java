...
public class NativeOrderComputeServiceImpl extends AbstractService implements NativeOrderComputeService {
     ...
     * (non-Javadoc)
     * @see
     * ca.firstvoices.publisher.services.NativeOrderComputeService#computeAssetNativeOrderTranslation(org.nuxeo.ecm.core
     * .api.DocumentModel)
     */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * ca.firstvoices.publisher.services.NativeOrderComputeService#computeDialectNativeOrderTranslation(org.nuxeo.ecm.
     * core.api.DocumentModel)
     */
    @Override
    ...
    protected void computeNativeOrderTranslation(DocumentModel[] chars, DocumentModel element) {
        ...
        while (title.length() > 0) {
            ...
            for (i = chars.length - 1; i >= 0; --i) {
                ...
                if ((charValue != null && title.startsWith(charValue))
                        || (ucCharValue != null && title.startsWith(ucCharValue))) {
                    nativeTitle += new Character(
                            (char) (33 + (Long) charDoc.getPropertyValue("fvcharacter:alphabet_order"))).toString();
